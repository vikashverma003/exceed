                    ...
                    <div class="row">
                        ...
                        <div class="col col-md-9">
                            <div class="result_found">
                                <div><div class="hide-banner">
                                  @if($category_count_banner==1)
                                    <div class="common_banner">
                                        <div class="banner-details position-relative">    
                                            <img  src="{{asset('uploads/categories/'.$product_banner_image->banner)}}">
                                            <div class="banner_title position-absolute d-flex align-items-end justify-content-between">
                                                <h3 class="text-uppercase m-0">{{$product_banner_image->name}}</h3>
                                                <span>Total Courses: {{$count_Course}}</span>
                                            </div>
                                        </div>
                                        <div class="baner-description p-3">
                                            <p>{{strip_tags($product_banner_image->desc)}}</p>
                                        </div>
                                    </div>
                                    @else
                                    @endif
                                </div>      
                                <div class="replace-banner" style="display:none">.</div></div>
                               <div class="search_res" style="margin:16px;"> @if(count($products_data) || count($manufacturer_data) || count($location_data) || count($category_data)) 
                                ...
                                <div class="sort_by-txt sub-category-list-menu selectedProducts">
                                   ...
                                   @foreach(@$products_data as $catrow)
                                        <span class="product-notify-span type1" title="Product Name">   {{strtoupper($catrow['name'])}}
                                            <!--<span class="RemoveProduct" data-name="{{str_slug($catrow['slug'])}}"></span>-->
                                        </span>
                                    @endforeach
                                     @endif
                                </div>
        ...
        const showSelectedProducts = function(data){
           // var search_fll="{{$search_flag}}";
            ...
            $.each(data.products_data, function(key, val){
                ...
                html+='<a href="{{url('/products-filter-course')}}/'+cat_id+'/'+course_name+'"><span class="product-notify-span type1">'+val.name+'<span class="" data-name="'+val.slug+'"></span></span></a>';
            })
            //$('.selectedProducts').html(html);
           // $('.selectedProducts').html("<p>Search Results For Courses:<p>"+html);
