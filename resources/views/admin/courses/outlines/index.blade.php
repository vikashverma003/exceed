...
@section('content')
    ...
    <div class="page-head">
        <div class="page-title">
            <h1>
                Manage Outlines For-{{ucfirst($course->name)}}
            </h1>
        </div>
    </div>
    ...
    <div class="portlet light md-shadow-z-2-i">
        <div class="portlet-title">
            <div class="caption">
                <a href="{{url('admin/outline/create/'.$course->id)}}" class="btn  btn-primary pull-right black" style="margin-top: 4px;margin-right: 5px;"><i class="fa fa-plus"></i>&nbsp;&nbsp; Add Outline</a>
            </div>
        </div>
        <div class="portlet-body">
            @include('admin.courses.outlines.table')
        </div>
    </div>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click','.change-status', function(e){
            ...
            if (result.value) {
                ...
                $.ajax({
                    async : false,
                    url: "{{url('admin/outline/status')}}", //url
        ...
        $(document).on('click','.delete-record', function(e){
            ...
            if (result.value) {
                ...
                $.ajax({
                    async : false,
                    url: "{{url('admin/outline/delete')}}", //url
                    type: 'post', //request method
                    data: {
                        'id':id
                    },
